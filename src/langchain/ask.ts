/**
 * CLI: ask the LangChain assistant a question.
 *
 *   npm run ask:lc -- "How does Xray scan for vulnerabilities?"
 *   npm run ask:lc -- "What is a virtual repository?" --product artifactory --top-k 6
 *   npm run ask:lc -- "Explain release bundles" --json
 */
...
async function main() {
  ...
  if (!args.question) {
    console.error('Usage: npm run ask:lc -- "your question" [--product xray] [--top-k 6] [--json]');
    process.exit(1);
  }
